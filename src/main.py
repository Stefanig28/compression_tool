...
from src import serialize, huffman_tree
...
def _compress(file: pathlib.Path) -> int:
    ...
    tree = huffman_tree.HuffmanTree.from_frequenceies(counts)
    table = cast(dict[str, str], huffman_tree.create_prefix_code_table(tree))
...
def _uncompress(file: pathlib.Path) -> int:
    ...
    tree = huffman_tree.HuffmanTree.from_frequenceies(frequencies)
